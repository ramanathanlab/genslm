...
class H5PreprocessMixin:
    ...
    def preprocess(
        ...
        if train_val_test_split is not None:
            ...
            split_length = sum(
                len(split) for split in sequence_splits.values()
            )
        ...
        with ExitStack() as stack:
            ...
            for i in tqdm(range(0, len(input_files), files_per_write)):
                ...
                for key, dset in h5_datasets.items():
                    ...
                    t_write = time.time()
                    dset[-inshape:] = all_dsets[
                        key
                    ]  # Single write of many in-h5 files
    ...
    def h5_to_fasta(
        ...
        read_h5_seq_desc = functools.partial(
            H5PreprocessMixin.read_h5_to_fasta_entries,
            num_slice=num_slice,
        )
